import { useEffect } from 'react';
import { useForm } from 'react-hook-form';
import { zodResolver } from '@hookform/resolvers/zod';
import { z } from 'zod';
import {
  Dialog,
  DialogContent,
  DialogHeader,
  DialogTitle,
  DialogDescription,
  DialogFooter,
} from '@/components/ui/dialog';
import {
  Select,
  SelectContent,
  SelectItem,
  SelectTrigger,
  SelectValue,
} from '@/components/ui/select';
import { Button } from '@/components/ui/button';
import { Input } from '@/components/ui/input';
import { DoorOpen, Save } from 'lucide-react';
import type { CompanyDto } from '@/types/masterData';
import type {
  GateDto,
  CreateGateRequest,
  UpdateGateRequest,
} from '@/types/infrastructure';

const gateSchema = z.object({
  companyId: z
    .string()
    .trim()
    .min(1, 'Vui lòng chọn công ty quản lý cổng'),
  code: z
    .string()
    .trim()
    .min(2, 'Mã cổng phải có ít nhất 2 ký tự')
    .max(50, 'Mã cổng không được quá 50 ký tự')
    .regex(/^[A-Za-z0-9_-]+$/, 'Mã cổng chỉ chứa chữ, số, gạch dưới hoặc gạch ngang'),
  name: z
    .string()
    .trim()
    .min(2, 'Tên cổng phải có ít nhất 2 ký tự')
    .max(100, 'Tên cổng không được quá 100 ký tự'),
  machineCode: z
    .string()
    .trim()
    .min(2, 'Mã máy bốt trực phải có ít nhất 2 ký tự')
    .max(100, 'Mã máy không được quá 100 ký tự'),
  isActive: z.boolean(),
});

type GateFormData = z.infer<typeof gateSchema>;

interface GateFormDialogProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  initialData?: GateDto | null;
  companies: CompanyDto[];
  onSubmit: (data: CreateGateRequest | UpdateGateRequest) => Promise<void>;
  isSubmitting?: boolean;
}

export function GateFormDialog({
  open,
  onOpenChange,
  initialData,
  companies,
  onSubmit,
  isSubmitting = false,
}: GateFormDialogProps) {
  const isEditing = Boolean(initialData);

  const {
    register,
    handleSubmit,
    reset,
    setValue,
    watch,
    formState: { errors },
  } = useForm<GateFormData>({
    resolver: zodResolver(gateSchema),
    defaultValues: {
      companyId: '',
      code: '',
      name: '',
      machineCode: '',
      isActive: true,
    },
  });

  const selectedCompanyId = watch('companyId');
  const isActive = watch('isActive');

  useEffect(() => {
    if (open) {
      if (initialData) {
        reset({
          companyId: initialData.companyId || '',
          code: initialData.code,
          name: initialData.name,
          machineCode: initialData.machineCode || '',
          isActive: initialData.isActive,
        });
      } else {
        reset({
          companyId: companies.length > 0 ? companies[0].id : '',
          code: '',
          name: '',
          machineCode: '',
          isActive: true,
        });
      }
    }
  }, [open, initialData, companies, reset]);

  const onFormSubmit = async (formData: GateFormData) => {
    await onSubmit({
      companyId: formData.companyId,
      code: formData.code.toUpperCase(),
      name: formData.name,
      machineCode: formData.machineCode,
      isActive: formData.isActive,
    });
  };

  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          <div className="flex items-center gap-2 mb-1">
            <div className="p-2 rounded-lg bg-blue-100 dark:bg-blue-950 text-blue-600 dark:text-blue-400">
              <DoorOpen className="h-5 w-5" />
            </div>
            <DialogTitle className="text-base font-bold">
              {isEditing ? 'Cập Nhật Thông Tin Cổng' : 'Thêm Mới Cổng Bãi Xe'}
            </DialogTitle>
          </div>
          <DialogDescription>
            {isEditing
              ? 'Chỉnh sửa thông tin cấu hình cổng kiểm soát và bốt trực.'
              : 'Chọn công ty quản lý và nhập thông tin cổng kiểm soát mới.'}
          </DialogDescription>
        </DialogHeader>

        <form onSubmit={handleSubmit(onFormSubmit)} className="space-y-3 py-1">
          {/* Chọn Công ty trực thuộc */}
          <div className="space-y-1">
            <label className="text-xs font-semibold text-foreground">
              Công ty quản lý <span className="text-destructive">*</span>
            </label>
            <Select
              value={selectedCompanyId}
              onValueChange={(val) => setValue('companyId', val, { shouldValidate: true })}
            >
              <SelectTrigger className="text-xs">
                <SelectValue placeholder="-- Chọn công ty quản lý --" />
              </SelectTrigger>
              <SelectContent>
                {companies.map((c) => (
                  <SelectItem key={c.id} value={c.id} className="text-xs">
                    {c.name} ({c.code})
                  </SelectItem>
                ))}
              </SelectContent>
            </Select>
            {errors.companyId && (
              <p className="text-[11px] text-destructive">{errors.companyId.message}</p>
            )}
          </div>

          {/* Mã cổng & Tên cổng */}
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div className="space-y-1">
              <label className="text-xs font-semibold text-foreground">
                Mã cổng <span className="text-destructive">*</span>
              </label>
              <Input
                {...register('code')}
                placeholder="VD: GATE_01"
                className="uppercase text-xs"
                autoFocus={!isEditing}
              />
              {errors.code && (
                <p className="text-[11px] text-destructive">{errors.code.message}</p>
              )}
            </div>

            <div className="space-y-1">
              <label className="text-xs font-semibold text-foreground">
                Tên cổng <span className="text-destructive">*</span>
              </label>
              <Input
                {...register('name')}
                placeholder="VD: Cổng Chính Số 1"
                className="text-xs"
              />
              {errors.name && (
                <p className="text-[11px] text-destructive">{errors.name.message}</p>
              )}
            </div>
          </div>

          {/* Mã máy bốt trực (MachineCode) */}
          <div className="space-y-1">
            <label className="text-xs font-semibold text-foreground">
              Mã máy bốt trực (MachineCode) <span className="text-destructive">*</span>
            </label>
            <Input
              {...register('machineCode')}
              placeholder="VD: BOT_BV_CHINH_01"
              className="text-xs font-mono"
            />
            <p className="text-[11px] text-muted-foreground">
              Mã định danh máy tính Client bốt bảo vệ kết nối điều khiển cổng.
            </p>
            {errors.machineCode && (
              <p className="text-[11px] text-destructive">{errors.machineCode.message}</p>
            )}
          </div>

          {/* Trạng thái hoạt động */}
          <div className="pt-2 border-t border-border flex items-center justify-between">
            <div>
              <span className="text-xs font-semibold text-foreground block">
                Trạng thái hoạt động
              </span>
              <span className="text-[11px] text-muted-foreground block">
                Cho phép các làn xe thuộc cổng này thực hiện kiểm soát
              </span>
            </div>
            <button
              type="button"
              onClick={() => setValue('isActive', !isActive, { shouldDirty: true })}
              className={`relative inline-flex h-5 w-9 shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none ${
                isActive ? 'bg-blue-600' : 'bg-muted'
              }`}
            >
              <span
                className={`pointer-events-none inline-block h-4 w-4 transform rounded-full bg-white shadow-lg ring-0 transition duration-200 ease-in-out ${
                  isActive ? 'translate-x-4' : 'translate-x-0'
                }`}
              />
            </button>
          </div>

          <DialogFooter className="pt-3 gap-2 sm:gap-0">
            <Button
              type="button"
              variant="outline"
              size="sm"
              onClick={() => onOpenChange(false)}
              disabled={isSubmitting}
              className="text-xs h-9 cursor-pointer"
            >
              Hủy bỏ
            </Button>
            <Button
              type="submit"
              size="sm"
              disabled={isSubmitting}
              className="text-xs h-9 bg-blue-600 hover:bg-blue-700 text-white cursor-pointer"
            >
              <Save className="h-3.5 w-3.5 mr-1.5" />
              <span>{isSubmitting ? 'Đang lưu...' : isEditing ? 'Cập nhật' : 'Thêm mới'}</span>
            </Button>
          </DialogFooter>
        </form>
      </DialogContent>
    </Dialog>
  );
}
