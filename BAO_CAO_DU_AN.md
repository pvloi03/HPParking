...
> **Ngày cập nhật:** 08/09/2026 (Đã đồng bộ sau nâng cấp .NET 8 & chuẩn hóa Domain Modeling)  
...
> **Bộ kiểm thử tự động:** 29 tests (xUnit + NSubstitute + FluentAssertions) - 100% Passed  
...
* **Kiểm thử tự động:** `HPParking.Tests` (xUnit 2.9.3, NSubstitute 5.3.0, FluentAssertions 7.0.0).
...
| **Bộ điều khiển Barrier & Tiếp nhận Wiegand** | ZKTeco Pull SDK (`plcommpro.dll`) | [ZKTecoSDK.cs](file:///c:/Users/ADMIN/source/repos/HPParking/HPParking/SDK/CtrlSDK/ZKTecoSDK.cs) & [ControllerService.cs](file:///c:/Users/ADMIN/source/repos/HPParking/HPParking/Services/Controller/ControllerService.cs): Kết nối TCP/IP, tiếp nhận chuỗi số điện thoại truyền từ FaceID qua cổng Wiegand, kích relay đóng/mở barrier (xem [ADR 0006](file:///c:/Users/ADMIN/source/repos/HPParking/docs/adr/0006-cardless-faceid-wiegand-architecture.md)). |
...
│       └── domain.md
│
├── HPParking.Tests/                # Dự án Kiểm thử tự động (29 tests - 100% Passed)
│   ├── Helper/ValidationHelperTests.cs
│   ├── LicenseKey/LicenseValidatorTests.cs
│   ├── Services/ImageStorageServiceTests.cs
│   ├── Models/LaneAndClientModelTests.cs
│   └── Services/ParkingWorkflowServiceTests.cs
│
...
3. **Đăng ký khách hàng & đồng bộ FaceID ([FrmRegisterClient.cs](file:///c:/Users/ADMIN/source/repos/HPParking/HPParking/Forms/FrmRegisterClient.cs)):**
...
| **Độ tin cậy & Kiểm thử** | ✅ Tuyệt đối | Bộ 29 unit tests xUnit bao phủ Helpers, Models, License, Storage và Workflow xe vào/ra chạy Passed 100% trong ~300ms. |
...
| **Độ ổn định giao diện & Khởi động** | ✅ Non-blocking | Loại bỏ hoàn toàn lỗi treo Form và lỗi tự đóng Form lúc khởi động; giao diện hiển thị tức thì kể cả khi thiết bị ngoại vi offline. |
| **Tài liệu & Mô hình nghiệp vụ** | ✅ Hoàn chỉnh | Đồng bộ hóa toàn diện giữa mã nguồn thực tế, từ điển thuật ngữ [CONTEXT.md](file:///c:/Users/ADMIN/source/repos/HPParking/CONTEXT.md) và 5 bản ghi quyết định kiến trúc [docs/adr/](file:///c:/Users/ADMIN/source/repos/HPParking/docs/adr/). |
